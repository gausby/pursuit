...
function Pursuit (query) {
    // A 'local' scope that will get passed around. This makes it
    // possible to pass around that current state and add convenience
    // functions to be used in the dictionary functions.
    var local = this || {};

    // The object of functions to use to generate source code.
    ...
    // The scope that will be refered to in the resulting run-time when
    // using `this` in the returned code.
    local.runTime = {};
    ...
    // The generated source code. If the compiler returned undefined the
    // resulting matching function will just return true on every input.
    // This can happen if the input is an empty object.
    var source = 'return ' + (compileQuery.call(local, query) || true);

    // Set debug to true if you need to check the source generated code.
    if (local.debug) {
    ...
    source
        ...
        .forEach(function(token) {
            if (token[0] && token[1]) {
                ...
                tokens[token[0]].push(
                    token.slice(1).filter(Boolean).join('&&')
                );
            }
            else {
