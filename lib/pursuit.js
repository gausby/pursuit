...
function Pursuit (query) {
    ...
    localScope.getScope = function (key) {
        key = key || localScope.key;
        if (localScope.scope) {
            return localScope.scope + '['+key+']';
        }
        else {
            return localScope.entry;
        }
    };

    var source = 'return ' + (compileQuery.call(localScope, query) || true);

    if (localScope.debug) {
        return 'function anonymous('+localScope.entry+') { '+source+' }';
    }
    else {
        /* jshint evil: true */
        return new Function(
...
function optimize (source, type) {
    ...
    tokens.map(function(item) {
        if (!isArray(foo[item[0]])) {
            foo[item[0]] = [];
        }

        foo[item[0]].push(item.slice(1).join('&&') || undefined);
        foo[item[0]] = foo[item[0]].filter(filterOutEmpty);
    });

    result = Object.keys(foo).sort(function(a, b) {
        return foo[a].length - foo[b].length;
    })
    .map(function(item) {
        if (foo[item].length === 0) {
            return item;
        }
        else if (foo[item].length === 1) {
            return [item, foo[item].join(type)].join('&&');
        }
        else {
            return [item,'('+foo[item].join(type)+')'].join('&&');
        }
...
function compileProperty (name, property, scope) {
    ...
    if (name === this.negation) {
        ...
        return '('+optimize(property.map(propertyArray, this), 'or').join('||')+')';
    }
        ...
        fns = Object.keys(property).map(function(key) {
            ...
            // nested properties
                ...
                if (property[key] instanceof RegExp || (typeof property[key] === 'function')) {
                    value = property[key];
                }
                else {
                ...
                if (key in this.dictionary) {
                    ...
                    if (typeof source === 'string') {
                        ...
                        // dictionary function returned something fishy
                        return undefined;
                    }
                }
                else {
        ...
        return (fns.length > 0) ? optimize(fns, 'and').join('&&') : undefined;
    }
