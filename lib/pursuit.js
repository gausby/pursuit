...
function Pursuit (query) {
    ...
    var source = 'return ' + compileQuery.call(localScope, query);
...
function compileProperty (name, prop, scope) {
    ...
    if (name === that.negation) {
        return '!('+compileProperty.call(that, undefined, prop, scope)+')';
    }
    ...
    fns = Object.keys(prop).map(function(key) {
        ...
        // nested properties
            ...
            if (key in that.dictionary) {
                ...
                that.key = name;
                return that.dictionary[key].call(that, value);
            }
            else {
                return false;
            }
