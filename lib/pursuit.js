...
function optimize (source, type) {
    ...
    var test = function (item) {
        return typeof item[0] === 'string' && item[0] === tokens[0][0];
    };
    ...
    if (tokens[0] && tokens[0][0] && tokens.every(test)) {
        var sub = tokens.map(function(item) {
            return item.slice(1).join('&&');
        });

        sub = sub.filter(filterOutEmpty);

        if (sub.length === 1) {
            tokens = [
                tokens[0][0]+'&&'+sub.join(type)
            ];
        }
        else {
            tokens = [
